...
internal object SocketEventListener : EventListener {
    ...
    private fun serializeAndBroadcast(event: Event) {
        ...
        val controller = httpServer.webSocketController ?: run {
            logger.error("WebSocket controller not initialized, skipping event broadcast")
            return
        }
        controller.broadcast(json) { _, t ->
