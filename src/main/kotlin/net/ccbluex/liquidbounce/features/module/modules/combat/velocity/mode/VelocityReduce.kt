...
object VelocityReduce : VelocityMode("Reduce") {
    ...
    private fun findTarget() {
        ...
        val killTarget = if (ModuleKillAura.running) ModuleKillAura.targetTracker.target else null
        if (killTarget != null) {
            if (lagTicks == -1) {
                renderTarget = killTarget
            }
            if (!canLag ||
                killTarget.squaredBoxedDistanceTo(player) <= lagTargetRange.start.sq()
            ) {
                target = killTarget
            }
            return
